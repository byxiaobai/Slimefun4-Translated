...
// Source File Name:   ToolListener.java
    ...
    implements Listener
{
    ...
    public void onBlockRegister(BlockPlaceEvent e)
    {
        ...
        if(sfItem != null && !(sfItem instanceof NotPlaceable))
        {
            ...
            if(SlimefunItem.blockhandler.containsKey(sfItem.getName()))
                ((SlimefunBlockHandler)SlimefunItem.blockhandler.get(sfItem.getName())).onPlace(e.getPlayer(), e.getBlock(), sfItem);
        } else
        {
            for(Iterator iterator = SlimefunItem.getHandlers("BlockPlaceHandler").iterator(); iterator.hasNext();)
            {
    ...
    public void onBlockPlace(BlockPlaceEvent e)
    {
        ...
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.ADVANCED_CIRCUIT_BOARD, true))
            e.setCancelled(true);
        else
        ...
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.ENDER_BACKPACK, false))
            e.setCancelled(true);
        else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.CARBON, false))
            e.setCancelled(true);
        else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.COMPRESSED_CARBON, false))
            e.setCancelled(true);
        else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.CARBON_CHUNK, false))
            e.setCancelled(true);
        else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.ANDROID_MEMORY_CORE, false))
            e.setCancelled(true);
        else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.LAVA_CRYSTAL, false))
            e.setCancelled(true);
        else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.TINY_URANIUM, false))
            e.setCancelled(true);
        else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.SMALL_URANIUM, false))
            e.setCancelled(true);
        else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.BROKEN_SPAWNER, false))
            e.setCancelled(true);
        else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.GPS_MARKER_TOOL, true))
        {
            e.setCancelled(true);
            Slimefun.getGPSNetwork().addWaypoint(e.getPlayer(), e.getBlock().getLocation());
        } else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.PRESENT, false))
        {
            e.setCancelled(true);
            ...
            e.getBlockPlaced().getWorld().dropItemNaturally(e.getBlockPlaced().getLocation(), (ItemStack)gifts.get(SlimefunStartup.randomize(gifts.size())));
        } else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.CARGO_INPUT, false))
        {
            if(e.getBlock().getY() != e.getBlockAgainst().getY())
            {
                Messages.local.sendTranslation(e.getPlayer(), "machines.CARGO_NODES.must-be-placed", true, new Variable[0]);
                e.setCancelled(true);
            }
        } else
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.CARGO_OUTPUT, false))
        {
            if(e.getBlock().getY() != e.getBlockAgainst().getY())
            {
                Messages.local.sendTranslation(e.getPlayer(), "machines.CARGO_NODES.must-be-placed", true, new Variable[0]);
        ...
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.CARGO_OUTPUT_ADVANCED, false))
        {
            if(e.getBlock().getY() != e.getBlockAgainst().getY())
            {
                Messages.local.sendTranslation(e.getPlayer(), "machines.CARGO_NODES.must-be-placed", true, new Variable[0]);
        ...
        if(SlimefunManager.isItemSimiliar(item, SlimefunItems.CT_IMPORT_BUS, false) && e.getBlock().getY() != e.getBlockAgainst().getY())
        {
            Messages.local.sendTranslation(e.getPlayer(), "machines.CARGO_NODES.must-be-placed", true, new Variable[0]);
            e.setCancelled(true);
        }
    ...
    public void onBlockBreak(BlockBreakEvent e)
    {
        ...
        SlimefunItem sfItem;
        if(StringUtils.equals(block2.getType().toString(), new String[] {
    "SAPLING", "WOOD_PLATE", "STONE_PLATE", "IRON_PLATE", "GOLD_PLATE"
}))
        {
            sfItem = BlockStorage.check(e.getBlock().getRelative(BlockFace.UP));
            if(sfItem != null && !(sfItem instanceof HandledBlock))
            {
                if(SlimefunItem.blockhandler.containsKey(sfItem.getName()))
                    allow = ((SlimefunBlockHandler)SlimefunItem.blockhandler.get(sfItem.getName())).onBreak(e.getPlayer(), block2, sfItem, UnregisterReason.PLAYER_BREAK);
                if(allow)
                {
        ...
        sfItem = BlockStorage.check(e.getBlock());
        if(sfItem != null && !(sfItem instanceof HandledBlock))
        {
            if(SlimefunItem.blockhandler.containsKey(sfItem.getName()))
        ...
        if(item != null)
        {
            if(item.getEnchantments().containsKey(Enchantment.LOOT_BONUS_BLOCKS) && !item.getEnchantments().containsKey(Enchantment.SILK_TOUCH))
            {
                ...
                if(fortune <= 0)
                    fortune = 1;
                fortune = (e.getBlock().getType() != Material.LAPIS_ORE ? 1 : 4 + SlimefunStartup.randomize(5)) * (fortune + 1);
            }
            for(Iterator iterator = SlimefunItem.getHandlers("BlockBreakHandler").iterator(); iterator.hasNext();)
            {
                ItemHandler handler = (ItemHandler)iterator.next();
        ...
        if(!item.getEnchantments().containsKey(Enchantment.SILK_TOUCH) && e.getBlock().getType().toString().endsWith("_ORE") && Talisman.checkFor(e, SlimefunItem.getByName("MINER_TALISMAN")))
        {
            if(drops.isEmpty())
                drops = (List)e.getBlock().getDrops();
            for(Iterator iterator1 = (new ArrayList(drops)).iterator(); iterator1.hasNext();)
            {
                ItemStack drop = (ItemStack)iterator1.next();
        ...
        if(!drops.isEmpty())
        {
            ...
            for(Iterator iterator2 = drops.iterator(); iterator2.hasNext();)
            {
                ItemStack drop = (ItemStack)iterator2.next();
                if(drop != null)
    ...
    {
        for(Iterator blocks = e.blockList().iterator(); blocks.hasNext();)
        {
            ...
            if(item != null)
            {
                ...
                if(!item.getName().equalsIgnoreCase("HARDENED_GLASS") && !item.getName().equalsIgnoreCase("WITHER_PROOF_OBSIDIAN") && !item.getName().equalsIgnoreCase("WITHER_PROOF_GLASS") && !item.getName().equalsIgnoreCase("FORCEFIELD_PROJECTOR") && !item.getName().equalsIgnoreCase("FORCEFIELD_RELAY"))
                {
                    ...
                    if(SlimefunItem.blockhandler.containsKey(item.getName()))
                        success = ((SlimefunBlockHandler)SlimefunItem.blockhandler.get(item.getName())).onBreak(null, block, item, UnregisterReason.EXPLODE);
                    if(success)
                    {
